...
import { leagueID, cplLeagueID, segundaLeagueID } from '$lib/utils/leagueInfo';
import { waitForAll } from './multiPromise';
import { get } from 'svelte/store';
import { upcomingDraft, previousDrafts } from '$lib/stores';
import { getLeagueRosters } from './leagueRosters';

export const getUpcomingDraft = async (queryLeagueID = leagueID) => {
	if (queryLeagueID === leagueID && get(upcomingDraft).draft) {
	...
	const [officialDraftRes, picksRes] = await waitForAll(
		...
		fetch(`https://api.sleeper.app/v1/league/${queryLeagueID}/traded_picks`, {
			compress: true
		})
	).catch((err) => {
		console.error(err);
...
const buildFromScratch = (
	...
	const progression =
		testRoster.wins + testRoster.ties + testRoster.losses;
	...
	for (const pick of picks) {
		...
		draft[pick.round - 1][
			draftOrder.indexOf(pick.roster_id.toString())
		] = pick.owner_id;
	}
...
const buildConfirmed = (
	...
	if (players && type != 'auction') {
		...
		for (const pick of picks) {
			...
			try {
				draft[pick.round - 1][
					draftOrder.indexOf(pick.roster_id)
				] = pick.owner_id;
			} catch (error) {
				console.error(
					`Possibly invaid roster ID?: ${pick.roster_id}`,
					error
				);
			}
		}
...
const completedNonAuction = ({
	...
	for (const playerData of players) {
		...
		draft[playerData.round - 1][
			playerData.draft_slot - 1
		] = { player };
	}
...
const completedAuction = ({
	...
	for (const roster in rosters) {
		...
		const sortedRoster = rosters[roster].sort(
...
// Previous drafts for a specific league
export const getPreviousDrafts = async (queryLeagueID = leagueID) => {
	if (queryLeagueID === leagueID && get(previousDrafts).length > 0) {
	...
	while (curSeason && curSeason != 0) {
		const [leagueData, completedDraftsInfo] =
			await waitForAll(
				getLeagueData(curSeason).catch((err) => {
					console.error(err);
				}),
				fetch(
					`https://api.sleeper.app/v1/league/${curSeason}/drafts`,
					{ compress: true }
				)
			).catch((err) => {
				console.error(err);
		...
		const completedDrafts =
			await completedDraftsInfo.json();
		...
		for (const completedDraft of completedDrafts) {
			...
			const [officialDraftRes, picksRes, playersRes] =
				await waitForAll(
					fetch(
			...
			const [officialDraft, picks, players] =
				await waitForAll(
					officialDraftRes.json(),
					picksRes.json(),
					playersRes.json()
			...
			const newDraft = {
				...
				draftType: officialDraft.type,
				reversalRound:
					officialDraft.settings.reversal_round
			};
...
// Get the Red and Green inaugural drafts
export const getCplDrafts = async () => {
	const [redDraft, greenDraft] = await waitForAll(
...
// Get previous Red and Green drafts
export const getCplPreviousDrafts = async () => {
	const [redDrafts, greenDrafts] = await waitForAll(
